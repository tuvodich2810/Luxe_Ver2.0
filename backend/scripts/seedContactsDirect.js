...
async function run() {
  try {
    await connectDB();
    console.log('Connected to DB. Seeding contacts...');
    await Contact.deleteMany({});
    const created = await Contact.create(sampleLeads);
    ...
    console.log(`✅ Thành công! Đã nạp ${created.length} hồ sơ khách hàng mẫu vào MongoDB Atlas!`);
    process.exit(0);
  } catch (err) {
